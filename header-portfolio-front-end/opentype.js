...
function hr(e, r) {
    ...
    var t = 0,
        ...
        n = 0,
        s = e.tag;
    do a = 2 * a + (s & 1), s >>>= 1, ++n, t += r.table[n], a -= r.table[n]; while (a >= 0);
...
g.OPERAND = function(e, r) {
    var t = [];
    if (Array.isArray(r))
        for (var a = 0; a < r.length; a += 1) U.argument(e.length === r.length, "Not enough arguments given for type" + r), t = t.concat(g.OPERAND(e[a], r[a]));
...
function Aa(e, r) {
    ...
    for (var a = 0; a < t; a += 1)
        for (var n = r.parseULong(), s = r.parseULong(), i = r.parseULong(), u = n; u <= s; u += 1) e.glyphIndexMap[u] = i, i++
}
...
function Ba(e, r, t, a, n) {
    ...
    for (var i = new k.Parser(t, a + n + 14), u = new k.Parser(t, a + n + 16 + s * 2), o = new k.Parser(t, a + n + 16 + s * 4), l = new k.Parser(t, a + n + 16 + s * 6), f = a + n + 16 + s * 8, h = 0; h < s - 1; h += 1)
        for (var p = void 0, c = i.parseUShort(), d = u.parseUShort(), x = o.parseShort(), m = l.parseUShort(), y = d; y <= c; y += 1) m !== 0 ? (f = l.offset + l.relativeOffset - 2, f += m, f += (y - d) * 2, p = k.getUShort(t, f), p !== 0 && (p = p + x & 65535)) : p = y + x & 65535, e.glyphIndexMap[y] = p
}
...
Cr.prototype.get = function(e) {
    if (this.glyphs[e] === void 0) {
        ...
        if (t)
            for (var a = 0; a < t.unicodes.length; a++) r.addUnicode(t.unicodes[a]);
...
function tn(e, r, t, a) {
    ...
    var u = [".notdef"],
        o = i.parseCard8();
    if (o === 0)
        for (var l = 0; l < t; l += 1) n = i.parseSID(), u.push(Oe(a, n));
    else if (o === 1)
        for (; u.length <= t;) {
...
function an(e, r, t) {
    var a, n = {},
        ...
        i = s.parseCard8();
    if (i === 0)
        for (var u = s.parseCard8(), o = 0; o < u; o += 1) a = s.parseCard8(), n[a] = o;
    else if (i === 1) {
        var l = s.parseCard8();
        a = 1;
        for (var f = 0; f < l; f += 1)
            for (var h = s.parseCard8(), p = s.parseCard8(), c = h; c <= h + p; c += 1) n[c] = a, a += 1
...
Ae.prototype = {
    ...
    expandCoverage: function(e) {
        ...
        for (var r = [], t = e.ranges, a = 0; a < t.length; a++)
            for (var n = t[a], s = n.start, i = n.end, u = s; u <= i; u++) r.push(u);
        return r
    }
};
...
K.prototype.getAlternates = function(e, r, t) {
    for (var a = [], n = this.getLookupTables(r, t, e, 3), s = 0; s < n.length; s++)
        for (var i = n[s].subtables, u = 0; u < i.length; u++)
            for (var o = i[u], l = this.expandCoverage(o.coverage), f = o.alternateSets, h = 0; h < l.length; h++) a.push({
                sub: l[h],
                by: f[h]
            });
    return a
    ...
    if (e.numberOfContours > 0) {
        ...
        for (var f = 0; f < l; f += 1)
            if (s = a.parseByte(), n.push(s), (s & 8) > 0)
                for (var h = a.parseByte(), p = 0; p < h; p += 1) n.push(s), f += 1;
...
function ra(e) {
    ...
    for (var t = Kn(e), a = 0; a < t.length; ++a) {
        ...
        for (var l = 0; l < n.length; ++l)
            if (s = i, i = u, u = n[(l + 1) % n.length], i.onCurve) r.lineTo(i.x, i.y);
            else {
                var f = s,
                    h = u;
                s.onCurve || (f = {
...
ia.prototype.exec = function(e, r) {
    ...
    if (!(this._errorState > 2)) {
        ...
        if (!a || a.ppem !== r) {
            ...
            var s = t.tables.cvt;
            if (s)
                for (var i = a.cvt = new Array(s.length), u = r / t.unitsPerEm, o = 0; o < s.length; o++) i[o] = s[o] * u;
            else a.cvt = [];
...
function ut(e, r) {
    ...
    do l !== n && s.setRelative(l, l, f, i), l = l.nextPointOnContour; while (l !== o)
}
...
var Mo = Object.freeze({
    ...
    Ho = Mo;
export {
    ne as Bidi, pe as BoundingBox, w as Font, Q as Glyph, P as Path, k as _parse, Ho as
    default, Bo as load, Io as loadSync, Dr as parse
};